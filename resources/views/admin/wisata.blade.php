...
@section('title', "Wisata - Admin Panel Hai Touna")
@section('content')
<!-- ============================================================== -->
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-title-desc">Silahkan Tambah Data Wisata Dengan Menekan Tombol Ini <button
                                data-toggle="modal" data-target=".modalTambahData" href=""
                                class="btn btn-primary btn-sm"><i class="bx bxs-add-to-queue"></i></button></p>
                        <table id="datatable-data-wisata" class="table table-striped table-bordered dt-responsive"
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="modalTambahData">Tambah Data Wisata</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="uploadDataWisata" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-0">
                    <div class="tombolSubmit">
                        <button id="tambahDataWisata" type="submit"
...
<div class="modal fade modalUbahData" tabindex="-1" role="dialog" aria-labelledby="modalUbahData"
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="modalUbahData">Ubah Data Wisata</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="uploadUbahDataWisata" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" id="id_wisata_ubah">
                    <div class="form-group">
                        <label>Judul</label>
                ...
                <div class="form-group mb-0">
                    <div>
                        <div class="tombolSubmit">
                            <button id="ubahDataWisata" type="submit"
                                class="btn btn-primary waves-effect waves-light mr-1">
                                Submit
                            </button>
                        </div>
...
<script>
    ...
    var table = $('#datatable-data-wisata').DataTable();
    tinymce.init({
    ...
    function getDataTable() {
            table.destroy();
            table = $('#datatable-data-wisata').DataTable({
                ajax:{
                    url: "{{URL('api/wisata/all') }}",
                },
                "columns": [
                    {
                        sortable: false,
                    ...
                    },
                    {
                        ...
                        "render": function (data, type, full, meta) {
                            return `
                                        <td>
                                            <button data-toggle="modal" data-target=".modalUbahData" id="lihatDataWisata" data-id="` + full.id_wisata + `" class="btn btn-info btn-small"><i
                                                    class="bx bx-paint d-line"></i></button>
                                            <button id="hapusDataWisata" data-id="` + full.id_wisata + `" class="btn btn-danger btn-small"><i class="bx bx-trash-alt"></i></button>
                                        </td>
                                    `;
                        }
                    },
    ...
    $('#tambahDataWisata').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status == "berhasil") {
                    Swal.fire(
                        'Status!',
                        'Data Wisata Berhasil Di Tambahkan!',
                        'success'
                    )
                    ...
                    Swal.fire(
                        'Status!',
                        'Data Wisata Gagal Di Tambahkan!',
                        'error'
                    )
                }
            }.bind($(this))
    ...
    $(document).on("click", '#hapusDataWisata', function () {
    ...
    $(document).on("click", '#lihatDataWisata', function () {
        var id_wisata = $(this).data("id");
        $.ajax({
            url: "{{URL('api/wisata/get') }}" + "/" + id_wisata,
            type: 'GET',
            dataType: "JSON",
            success: function (response) {
                $('#judul_ubah').val(response.judul);
                $('#id_wisata_ubah').val(response.id_wisata);
    ...
    $('#ubahDataWisata').on('click', function () {
        let formData = new FormData();
        var keterangan_ubah = tinymce.activeEditor.getContent();
        var id_wisata_ubah = $('#id_wisata_ubah').val();
        ...
        $.ajax({
            ...
            cache: false,
            processData: false,
            url: "{{URL('api/wisata/change') }}" + "/" + id_wisata_ubah,
            beforeSend: function(){
                $('.loader').show();
                $('.tombolSubmit').hide();
            },
